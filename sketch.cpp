...
void loop()
{
  ...
  if(nextAppointmentByteAddress == appointmentLastByteAddress){ //if there are no more appointments do not come into this section again.
...
void getNextUnExpiredAppointment(){
  ...
  int oldMin = minute();
  int appointmentYear;
  unsigned long appointmentTimeStampWithoutYear;
...
void getTimeStampFromAppointment(){// this populates the timeStamp string
  ...
  if ( 0 == noMoreAppointments ){
    ...
    Serial.println("No New Appointments, nothing to get Time stamp from.");
  }
}
...
void ReadNextAppointment(){
    ...
    //not initialized to zero everytime so that the count is maintained.
  {
    ...
    if(c == '~')
    {
      ...
      //Serial.println();
      nextAppointmentByteAddress++;//set nextAppointmentByteAddress to point to the start of the next appointment message
